...
{
  my @r = validate_dependencies(sub {});
  $r[1]->{callbacks}->{autogenerated} = 'foo';
  is_deeply(
    [@r],
    [_wfbiobowcv => { optional => 1, callbacks => { autogenerated => 'foo' } }],
    "validate_dependencies(sub{}) returns the right stuff"
  );
  @r = validate_dependencies('puppies', sub {});
  $r[1]->{callbacks}->{autogenerated} = 'foo';
  is_deeply(
    [@r],
    [puppies => { optional => 1, callbacks => { autogenerated => 'foo' } }],
...
dies_ok(sub { foo() }, 'any_of', "bad params, any_of fails");
